...
export async function getLeaderboard() {
  return await db
    ...
    `.as("totalPoints"),
      ...
      totalNftsSold:
        sql<number>`COALESCE(COUNT(DISTINCT CASE WHEN ${listings.status} = 'sold' THEN ${listings.id} END), 0)`.as(
          "totalNftsSold"
        ),
      // userBalance: sql<number>`COALESCE(SUM(DISTINCT ${balances.balance}), 0)`.as("userBalance"),
      totalLikes: sql<number>`COALESCE(COUNT(DISTINCT likes.id), 0)`.as(
        "totalLikes"
      ),
      totalNftsMinted: sql<number>`COALESCE(COUNT(DISTINCT ${nfts.id}), 0)`.as(
        "totalNftsMinted"
      ),
      totalEarnings:
        sql<number>`COALESCE(SUM(DISTINCT CASE WHEN ${listings.status} = 'sold' THEN ${listings.price} END), 0)`.as(
          "totalEarnings"
        ),
    })
    .from(users)
    ...
    .leftJoin(
      likes,
      sql`${likes.listingId} = ${listings.id} AND ${listings.seller} = ${users.address}`
    )
